...
import { ChevronLeft, Check, Package, Truck, Shield } from "lucide-react"
...
export function ProductPageClient({ product, details }: ProductPageClientProps) {
  ...
  const [selectedSize, setSelectedSize] = useState(details.sizes[0])
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-3xl lg:text-4xl font-bold text-accent mb-6">{selectedSize.price}</p>
              <p className="text-muted-foreground leading-relaxed text-lg">{productDesc}</p>
            </div>

            <div className="space-y-4">
              <h3 className="font-semibold text-lg">{language === "fi" ? "Valitse koko" : "Select size"}</h3>
              <div className="grid grid-cols-2 gap-3">
                {details.sizes.map((size) => (
                  ...
                  >
                    ...
                    <div className="text-sm font-medium mt-1">{size.price}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 py-6 border-y">
              ...
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {language === "fi" ? "Huolellinen pakkaus" : "Careful packaging"}
                </span>
              </div>
            </div>
